...
% model dosyasinin hangi kismindan parametrelerin okunacagi main fonksiyonundan gelen 'baslangic' ve 'son' degerleri ile belirlenir
% model dosyasi acilir ve esitliklerde kullanilacak tum parametreler okunur
for i=baslangic:son
...
%if step==0
% -------------------------- Denklemler -----------------------------------
% bu denklemlerin tamami BSIM3 manualinden alinmistir herhangi bir degisiklik yapilmamistir
nch = nch*10^6;
% i=1;
% for vd=0:0.01:1.8
vgs = vg - vs;
...
Vt = k1*(temp+273.15)/q;
phi_s = 2*k1*(tnom+273.15)/q*log(nch/10^6/ni);
...
Vgsteff = (2*n*Vt*log(1+exp((vgs-vth_app)/(2*n*Vt))))/(1+2*n*Cox*sqrt((2*phi_s)/(q*Esi*nch))*exp(-(vgs-vth_app-2*voff)/(2*n*Vt)));
%Vgsteff = (2*n*Vt*log(1+exp((vgs-vth_app)/(2*n*Vt))))/(1+2*n*(Cox/Cdepo)*exp(-(vgs-vth_app-2*voff)/(2*n*Vt)));
...
Rds = (Rdsw*(1+prwg*Vgsteff+prwb*(sqrt(phi_s-Vbseff)-sqrt(phi_s))))/((10^6*Weff1)^wr);
...
% FITTING FUNCTIONS
%[ratio, distance ] = width_length_shifting( vgs, vds, vbs, Wdrawn, Ldrawn, temp );
...
if vbs == 0 && level == 1
    ...
    %hedef vth degerine ulasmak icin kt1'in ne kadar degistirilmesi gerektigi hesaplanir
    const2 = vth-vth0_n-(kt1_n + kt1l_n/Leff + kt2_n*Vbseff)*(c_temp-1);
    kt1_n_app2 = ((vth_app-const2)-vth0_n)/(c_temp-1)-kt1l_n/Leff-kt2_n*Vbseff
...
% AT degerindeki ufak degisiklikler Vdsat uzerine cok fazla etki gosterebiliyor ve hesaplama ile elde edilen AT degeri cok fazla farklilik gosteriyor bu yuzden gecici olarak devre disi birakildi
%%%%%% AT %%%%%%%%
% if vbs == 0 && level == 1
...
%
% eqn = vdsat + vdsat_difference - Vdsat;
...
%%%%%%% UTE %%%%%%%
% UTE parametresinin akim denklemlerine ilk dahil oldugu noktadan itibaren denklemlere UTE bilinmeyen olarak verilir ve bu denklemin sonucu hedef akim degerine esitlenerek UTE bilinmeyeni bulunur
if vbs == 0 && level == 1
    ...
    if isempty(ute_app) % UTE cozdurulemezse denklemi basitlestirerek tekrar dene
        syms ute_app
        ute_app = double(solve(simplify(eqn),ute_app));
        if isempty(ute_app) % UTE yine bulunamazsa baslangictaki UTE degerini kaydirip denklemleri tekrar olusturarak hedef akim icin UTE degeri bulunmaya calisilir
    ...
    %%%%%%%%% UB1 %%%%%%%%%
    % bu parametre ID-VGS egrisinde sicakliktan kaynakli bozulmalari duzeltmek icin kullanilir ve iterasyonlarla bulunur denklem cozdurulmez
    while max(abs(diff(I)))>mean(abs(diff(I((size(I,1)-20):size(I,1)))))*1.7
        if abs(ub1_n)<abs(ub1_n_max)
            min_diff = min(diff(I,2));
            ub1_n = ub1_n+(ub1_n_max-ub1_n_def)/step;
...
%%%%%%% KT2 %%%%%%%
% bulunma mantigi KT1 ile ayni sadece body bias oldugu zaman aktif hale geliyor
if vbs < 0 && level == 2
    const2 = vth-vth0_n-(kt1_n + kt1l_n/Leff + kt2_n*Vbseff)*(c_temp-1);
...
%%%%%%% UC1 %%%%%%%
% UTE ile ayni sekilde hesaplaniyor, body bias oldugu zaman kullaniliyor
if vbs < 0 && level == 2
    [Ids_boyut,~] = size(Ids);
    %ratio=Ids(Ids_boyut,1)/Ids_app;
    %ratio_vdsat = vdsat/Vdsat_app;
    ...
    %%%%%%%%% UB1 in UC1 %%%%%%%%%%%
    % UC1 degistiginde ID-VGS egrisinde tekrardan bozulma gozlenebiliyor bu yuzden tekrar UB1 parametresi kontrol ediliyor
    while max(abs(diff(I)))>mean(abs(diff(I((size(I,1)-20):size(I,1)))))*1.7
        if abs(ub1_n)<abs(ub1_n_max)
            min_diff = min(diff(I,2));
            ub1_n = ub1_n+(ub1_n_max-ub1_n_def)/step;
